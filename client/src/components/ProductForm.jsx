    ...
    if (file) {
      ...
      try {
        ...
        const cloudinaryResponse = await axios.post(
          "https://api.cloudinary.com/v1_1/dbg7wcrip/image/upload",
          formData,
          {
            headers: {
        ...
        changeLink(cloudinaryResponse.data.secure_url,index);
    ...
    <form className="order-table">
      <div className="dash-input-1">
        ...
        <div className="admin-upload">
          {[1, 2, 3, 4].map((index) => (
            <label htmlFor={`dropzone-file-${index}`} key={index} className="grid-item">
              <div>
                {linkData[`link${index}`]!==null ? (
                  <img src={linkData[`link${index}`]} alt="Preview" />
                ) : (
                  <>
                    <svg
                      aria-hidden="true"
